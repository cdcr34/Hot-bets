...
# --- Original Odds Input with +/- and flip button ---
...
with col3:
    if st.button("➕", key="orig_plus"):
        st.session_state.original_odds += 1
with col4:
    if st.button("±", key="orig_flip"):
        st.session_state.original_odds *= -1

original_odds = st.session_state.original_odds

# --- New Odds Input with +/- and flip button ---
st.markdown("**New Odds (your current odds):**")
col5, col6, col7, col8 = st.columns([1, 2, 1, 1])
with col5:
